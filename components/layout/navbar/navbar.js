...
export class Navbar extends Component {
    ...
    async _render() {
        console.log('Navbar _render() called');

        try {
            const res = await fetch(this.url + '?raw');
            const htmlText = await res.text();
            console.log('Navbar template fetched');

            const tpl = stripScripts(htmlText);

            if (tpl.content) {
                console.log('Filtering by role...');
                await this.filterByRole(tpl.content);
            }

            this.host.innerHTML = '';
            this.host.append(tpl.content.cloneNode(true));
            console.log('Navbar content injected');

            await this.injectProfilePicture();
            console.log('Profile picture injected');
